...
def normalize_subject(s):
    ...
    replacements = {
        'spl div': 'special dividend', 'sd': 'special dividend', 'dvd': 'dividend', 'div': 'dividend',
        'spl': 'split', 'sub div': 'split', 'sub-division': 'split', 'subdivision': 'split',
        'bon': 'bonus', 'bns': 'bonus', 'rts': 'rights', 'right': 'rights'
    }
    for k, v in replacements.items():
        s = s.replace(k, v)
    return s
...
# --- NEW: Helper to get Price P for Rights Issue ---
def get_cum_rights_close_price(symbol, ex_date, folder_path):
    """
    Fetches the Close Price (P) on the last date BEFORE the Ex-Date.
    """
    path = os.path.join(folder_path, f"{symbol}.csv")
    if not os.path.exists(path):
        return None
    
    try:
        df = pd.read_csv(path)
        # Handle flexible date columns
        ...
        # Filter for dates strictly BEFORE Ex-Date
        mask = df[date_col] < pd.to_datetime(ex_date)
        prev_data = df[mask]
        
        if prev_data.empty: return None
        
        # Return the last Close price
        if 'Close' in prev_data.columns: return prev_data.iloc[-1]['Close']
        if 'CLOSE_PRICE' in prev_data.columns: return prev_data.iloc[-1]['CLOSE_PRICE']
...
def identify_action(subject, face_val):
    ...
    # STOP words (Removed 'rights' from here)
    stop_words = ['demerger', 'amalgamation', 'scheme', 'capital reduction', 'consolidation', 'nclt']
    if any(w in s for w in stop_words):
        return 'STOP', None
    ...
    # --- RIGHTS ISSUE (New Logic) ---
    if 'rights' in s:
        # 1. Try to find Ratio A:B (e.g., 1:4)
        m_ratio = re.search(r'(\d+)\s*:\s*(\d+)', s)
        
        # 2. Try to find Subscription Price S (e.g., @ 150 or at 150)
        # Looks for numbers following @, at, or rs
        m_price = re.search(r'(?:@|at|rs\.?)\s*(\d+(?:\.\d+)?)', s)
        
        if m_ratio and m_price:
            A = float(m_ratio.group(1)) # New Shares
            B = float(m_ratio.group(2)) # Existing Shares
            S = float(m_price.group(1)) # Offer Price
            
            # Return parsed params to be used in Phase 1
            return 'RIGHTS', {'A': A, 'B': B, 'S': S}

    return 'IGNORE', None

# ================= PHASE 1: INTERACTIVE RESOLUTION =================

def resolve_actions_interactively(ca_path, log_path, cash_data_folder):
    print(f"\n🔍 [Phase 1] Scanning Corporate Actions from: {os.path.basename(ca_path)}")
    ...
    for idx, row in df_ca.iterrows():
        ...
        action, result = identify_action(subject, face_val)
        factor = None
        
        # --- RIGHTS ISSUE CALCULATION LOGIC ---
        if action == 'RIGHTS':
            # result contains {'A': A, 'B': B, 'S': S}
            A = result['A']
            B = result['B']
            S = result['S']
            
            # Fetch Cum-Rights Price (P) automatically
            P = get_cum_rights_close_price(symbol, exdate_str, cash_data_folder)
            
            if P is not None:
                # Formula: TERP = ((P * B) + (S * A)) / (B + A)
                terp = ((P * B) + (S * A)) / (B + A)
                
                # Formula: Factor = TERP / P
                factor = terp / P
                
                print(f"   ℹ️ Auto-Calced Rights for {symbol}: P={P}, S={S}, Ratio={A}:{B} -> Factor={factor:.4f}")
            else:
                print(f"   ⚠️ Rights detected for {symbol} but could not fetch Price (P) from CSV.")
                action = 'STOP' # Fallback to manual input
        
        elif action in ['SPLIT', 'BONUS', 'DIVIDEND']:
            factor = result

        # Check if intervention is needed
        needs_input = (action == 'STOP') or (s_cash == 'Unadjusted') or (s_fut == 'Unadjusted') or (factor is None and action != 'IGNORE')
        ...
        if needs_input:
            ...
            print(f"📌 Symbol: {symbol} | Date: {exdate_str}")
            print(f"📝 Subject: {subject}")
            print(f"🔄 Detected: {action} | Auto-Factor: {factor if factor else 'None'}")
            print("-" * 60)
            print("   [1] Enter Factor (Adjust)")
            print("   [2] Skip (Mark as Unadjusted)")
            print("   [3] REMOVE (Delete from CA File)")
...
def process_market_data(mode, data_dir, approved_factors, log_queue):
    ...
    # NEW: Create a set of symbols that actually need adjustment
    target_symbols = set() 
    ...
    for (sym, date), data in approved_factors.items():
        ...
        target_symbols.add(sym) # Add to optimization set

    count = 0
    
    # NEW: Loop ONLY through symbols that have actions, instead of scanning the whole directory
    for symbol in target_symbols:
        file_name = f"{symbol}.csv" # Assuming file naming is SYMBOL.csv
        path = os.path.join(data_dir, file_name)
        
        # Check if file exists (since some stocks might be in Cash but not Futures)
        if not os.path.exists(path):
            continue

        matched_actions = symbol_actions[symbol]
        ...
        try:
            ...
            for item in matched_actions:
                ...
                # Apply Volume (Splits/Bonus/Rights)
                # Rights also affect volume usually (contract size increases), but price adjustment is primary.
                # If strictly following user instruction: "New Price = Old Price / Adjustment Factor"
                # The user instruction said: "New Lot = Old Lot * Adjustment Factor". 
                # Since we deal with raw OHLC data here, dividing price is equivalent to multiplying price by (TERP/P) which is < 1.
                # For volume, we divide by factor (since price went down, volume/qty goes up).
                
                if action in ['SPLIT', 'BONUS', 'RIGHTS', 'MANUAL']: 
                    vol_cols = ['OPEN_INT', 'TRD_QTY', 'NO_OF_CONT'] if mode == 'FUTURES' else ['Volume']
                    existing_vol = [c for c in vol_cols if c in df.columns]
                    for col in existing_vol:
                        df.loc[mask, col] /= factor
                ...
                log_queue.put({'symbol': symbol, 'date': item['date_str'], 'type': mode})
...
def run_engine():
    ...
    # --- PHASE 1 (Updated args to pass Cash Data folder for Rights Price fetch) ---
    factors = resolve_actions_interactively(ca_path, PATHS["LOG_FILE"], PATHS["STOCK_WISE_PR"])
...
if __name__ == "__main__":
    # run_engine()
    print(DATA_DIR)
    print(PATHS["STOCK_WISE_PR"])
    print(PATHS["STOCK_WISE_FO"])
    print(PATHS["CLEAN_CA"])
