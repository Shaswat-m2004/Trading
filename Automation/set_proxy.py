...
# # # C:\Users\91702\Desktop\full_and_final\Futures_data
...
# #         fut_df['TIMESTAMP'] = pd.to_datetime(fut_df['TIMESTAMP'],dayfirst=True, errors='coerce')
...
# # Folder Paths
# CURRENT_DIR = r"C:\Users\91702\Desktop\full_and_final"
# FUTURES_FOLDER = os.path.join(CURRENT_DIR, "Futures_data")
# CASH_FOLDER    = os.path.join(CURRENT_DIR, "Cash_data")
# NIFTY_FILE     = os.path.join(FUTURES_FOLDER, "NIFTY.csv")
...
#         # FIX: Removed dayfirst=True because format is YYYY-MM-DD
...
#         # Ensure all required columns exist in cash file
...
#         # FIX: Removed dayfirst=True to stop warnings (Format is YYYY-MM-DD)
#         fut_df['TIMESTAMP'] = pd.to_datetime(fut_df['TIMESTAMP'], errors='coerce')
#         fut_df['EXP_DATE']  = pd.to_datetime(fut_df['EXP_DATE'], errors='coerce')
...
#     # --- D. Gap Detection & Filling Logic ---
#     # FIX: Using 'CLOSE' instead of 'CLOSE_PRICE'
    
#     # Calculate 'Basis' (Difference between Future and Cash)
#     merged['BASIS'] = merged['CLOSE'] - merged['C_CLOSE']
...
#     # --- F. Reconstruct Synthetic OHLC ---
#     # Identify rows that need filling (where CLOSE is NaN)
#     mask = merged['CLOSE'].isna()
    
#     if not mask.any():
#         return None # No gaps found

#     # FIX: Using 'CLOSE', 'OPEN', 'HIGH', 'LOW' names
#     merged.loc[mask, 'CLOSE'] = merged.loc[mask, 'C_CLOSE'] + merged.loc[mask, 'BASIS']

#     # Fut_High = Fut_Close + (Cash_High - Cash_Close)
#     merged.loc[mask, 'OPEN'] = round(merged.loc[mask, 'CLOSE'] + (merged.loc[mask, 'C_OPEN'] - merged.loc[mask, 'C_CLOSE']), 2)
#     merged.loc[mask, 'HIGH'] = round(merged.loc[mask, 'CLOSE'] + (merged.loc[mask, 'C_HIGH'] - merged.loc[mask, 'C_CLOSE']), 2)
#     merged.loc[mask, 'LOW']  = round(merged.loc[mask, 'CLOSE'] + (merged.loc[mask, 'C_LOW']  - merged.loc[mask, 'C_CLOSE']), 2)
    
#     # Sanity check: Low Price > 0
#     merged.loc[mask & (merged['LOW'] <= 0), 'LOW'] = 0.05

#     # Fill Zero Values for Volume/OI
#     cols_to_zero = ['VOLUME', 'OPEN_INT', 'CONTRACTS', 'TURNOVER']
...
# Folder Paths
CURRENT_DIR = r"C:\Users\91702\Desktop\full_and_final"
dir = r"C:\Users\91702\Documents\programming\all_cash_stocks\Trading\Futures_data"
# FUTURES_FOLDER = os.path.join(CURRENT_DIR, "Futures_data")
FUTURES_FOLDER = dir
CASH_FOLDER    = os.path.join(CURRENT_DIR, "Cash_data")
NIFTY_FILE     = os.path.join(FUTURES_FOLDER, "NIFTY.csv")
LOG_FILE       = os.path.join(CURRENT_DIR, "harmonization_log.txt")
...
def setup_logger():
    ...
    logging.basicConfig(
        ...
        datefmt='%Y-%m-%d %H:%M:%S',
        filemode='w'  # 'w' overwrites the log every time. Change to 'a' to append.
    )
    console = logging.StreamHandler()
    console.setLevel(logging.WARNING) # Only print warnings/errors to console to keep TQDM clean
...
def load_nifty_master_schedule():
    ...
    """
    print("⏳ Building Master Timeline from Nifty...")
    logging.info("Starting process: Building Master Timeline from Nifty...")
    
    try:
        if not os.path.exists(NIFTY_FILE):
            msg = f"NIFTY file not found at: {NIFTY_FILE}"
            print(f"❌ {msg}")
        ...
        df = df.dropna().drop_duplicates().sort_values(['TIMESTAMP', 'EXP_DATE'])
        logging.info(f"Nifty Master Schedule loaded. Rows: {len(df)}")
        return df
    except Exception as e:
        msg = f"Critical Error loading Nifty: {e}"
        print(f"❌ {msg}")
        logging.critical(msg)
        return pd.DataFrame()
...
def process_single_stock(file_path, master_schedule):
    """
    Returns a tuple: (Status_String, Message_for_Log)
    """
    symbol = os.path.basename(file_path).replace(".csv", "")
    if symbol.upper() == "NIFTY": 
        return "SKIPPED", f"{symbol}: Skipped (Master File)"

    # --- A. Load Data ---
    try:
        cash_df = load_cash_fast(symbol)
        if cash_df is None or cash_df.empty: 
            return "SKIPPED", f"{symbol}: No matching Cash data found."
        ...
        return "CORRUPT", f"{symbol}: Error reading file - {str(e)}"
    ...
    try:
        ...
        if not mask.any():
            return "CLEAN", f"{symbol}: No gaps found."

        # Fill Close
        merged.loc[mask, 'CLOSE'] = merged.loc[mask, 'C_CLOSE'] + merged.loc[mask, 'BASIS']

        # Fill OHLC based on Cash movement + Basis
        merged.loc[mask, 'OPEN'] = round(merged.loc[mask, 'CLOSE'] + (merged.loc[mask, 'C_OPEN'] - merged.loc[mask, 'C_CLOSE']), 2)
        merged.loc[mask, 'HIGH'] = round(merged.loc[mask, 'CLOSE'] + (merged.loc[mask, 'C_HIGH'] - merged.loc[mask, 'C_CLOSE']), 2)
        merged.loc[mask, 'LOW']  = round(merged.loc[mask, 'CLOSE'] + (merged.loc[mask, 'C_LOW']  - merged.loc[mask, 'C_CLOSE']), 2)
        ...
        gaps_filled = mask.sum()
        return "FIXED", f"{symbol}: Success. Filled {gaps_filled} gaps."
    
    except Exception as e:
        return "ERROR", f"{symbol}: Logic Error - {str(e)}"
...
def harmonize_futures_safely():
    ...
    master_schedule = load_nifty_master_schedule()
    if master_schedule.empty: 
        logging.critical("Master Schedule is empty. Exiting.")
        return

    # 2. Get Files
    files = glob.glob(os.path.join(FUTURES_FOLDER, "*.csv"))
    msg_start = f"🚀 Starting Parallel Harmonization for {len(files)} stocks..."
    print(msg_start)
    ...
    stats = {"FIXED": 0, "CORRUPT": 0, "SKIPPED": 0, "CLEAN": 0, "ERROR": 0}

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        worker = partial(process_single_stock, master_schedule=master_schedule)
        
        # Process results as they complete
        # We iterate over results to log them safely in the main process
        results = list(tqdm(executor.map(worker, files), total=len(files)))
        
        for status, log_message in results:
            if status in stats:
                stats[status] += 1
            ...
            if status == "FIXED":
                ...
                logging.info(log_message) # Clean or Skipped

    # 4. Summary
    print("\n" + "="*40)
    print(f"✅ Processing Complete. Check log at: {LOG_FILE}")
    print(f"🔹 Files Updated (Gaps Filled): {stats['FIXED']}")
    print(f"🔹 Files Clean (No Gaps):       {stats['CLEAN']}")
    print(f"🔹 Files Skipped (No Cash Data):{stats['SKIPPED']}")
    print(f"🔹 Files Corrupt/Errors:        {stats['CORRUPT'] + stats['ERROR']}")
    print("="*40)

    logging.info("="*40)
    logging.info("PROCESSING SUMMARY")
    logging.info(f"Files Updated: {stats['FIXED']}")
    logging.info(f"Files Clean:   {stats['CLEAN']}")
    logging.info(f"Skipped:       {stats['SKIPPED']}")
    logging.info(f"Errors:        {stats['CORRUPT'] + stats['ERROR']}")
    logging.info("="*40)
